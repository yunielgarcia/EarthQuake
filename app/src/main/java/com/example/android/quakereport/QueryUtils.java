...
public final class QueryUtils {
    ...
    public static ArrayList<EarthQuake> extractEarthquakes() {
        ...
        try {
            ...
            // list of earthquakes
            JSONObject request = new JSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray earthquakesList = request.getJSONArray("features");

            for (int i = 0; i < earthquakesList.length(); i++) {
                JSONObject currentQuakeG = earthquakesList.getJSONObject(i);
                JSONObject currentQuake = currentQuakeG.getJSONObject("properties");

                //Magnitude
                Double magnitude = currentQuake.getDouble("mag");

                //Location
//                String[] locationArr = currentQuake.getString("place").split(",");
                String location = currentQuake.getString("place");
//                String location = locationArr[locationArr.length - 1];

                //Date
                Date dateObj = new Date(currentQuake.getLong("time"));
